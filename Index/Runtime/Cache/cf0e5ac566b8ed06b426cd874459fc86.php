<?php if (!defined('THINK_PATH')) exit(); require APP_ROOT.'public/member_top.php';?>
...
<!--header-box-->
<?php
$order_id=pg('order_id'); if($order_id=='') { $data=$_GET; if($data['is_success']==TRUE) { $order=unserialize($data['metadata']); } } else { $order=M('order')->where(array('order_id'=>$order_id))->find(); } ?>
