...
<div class="content">
	...
	<?php
 $type_id = pg('type_id'); $classify_id = pg('classify_id'); $content_id = pg('content_id'); $table_name = M('classify_type')->where(array('type_id' => $type_id))->getField('table_name'); ?>
 ...
 $where = array(); $where['type_id'] = $type_id; $where['list_switch'] = 2; $list_input=M('input')->where($where)->order('date asc')->select(); foreach($list_input as $k=>$v){ ?>
				...
				<?php
 $perpage=20; $offset=($p-1)*$perpage; $where = array(); $where['r.classify_id'] = $classify_id; $where['r.type_id'] = $type_id; $list = M($table_name)->where(array('recruit_id' => $content_id))->order('date desc')->limit($offset,$perpage)->select(); $total_num = M($table_name)->where(array('recruit_id' => $content_id))->order('date desc')->count(); foreach($list as $k=>$v){ $v['content_id']=$v[$table_name.'_id']; ?>
					<tr>
						<td height="35">
 ...
 $input = M('input')->where(array('input_pid' => $val['input_id']))->order('date asc')->select(); foreach($input as $k2=>$v2){ ?>
						...
						<td>
							<?php
 $relevance_list = M('relevance')->where(array('content_id' => $v['content_id'], 'type_id' => $type_id))->order('main_id desc')->select(); foreach($relevance_list as $key=>$val){ $classify_list = M('classify')->where(array('classify_id' => $val['classify_id']))->select(); if($val['main_id'] == 1){ echo '<font style="font-weight:bold;">'.$classify_list[0]['classify_name'].'</font>'; }else{ echo "|".$classify_list[0]['classify_name']; } } ?>
						</td>
