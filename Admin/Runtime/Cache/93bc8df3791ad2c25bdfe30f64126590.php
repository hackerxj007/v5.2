...
<div class="content">
	...
	<?php
 $type_id = 46; $export_content_id=pg('export_content_id'); $table_name = M('classify_type')->where(array('type_id' => $type_id))->getField('table_name'); ?>
...
<!--                    <td>审核</td>-->
				...
				<?php
 $perpage=20; $offset=($p-1)*$perpage; $list = M($table_name)->limit($offset,$perpage)->order('date desc')->select(); $total_num = M($table_name)->count(); foreach($list as $k=>$v){ ?>
					<tr>
...
<!--									余额只能为正值-->
						...
						<?php }?>
                        <td><?php $balance = M('account')->where(array('member_id' => $v['member_id']))->order('account_id desc')->getField('balance');if($balance=='')$balance=0; echo $balance;?>&nbsp;<a class="remains" href="admin.php?m=member&a=account&member_id=<?php echo $v['member_id'];?>">明细</a></td>
