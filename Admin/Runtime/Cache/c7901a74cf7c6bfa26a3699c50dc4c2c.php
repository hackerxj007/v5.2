...
<div class="content">
  ...
  <?php
 $data=pg('data'); $search_type=pg('search_type'); $search=trim(pg('search')); $lt_date=pg('lt_date'); $gt_date=pg('gt_date'); $data[$search_type]=$search; if($lt_date && $gt_date) { $where['date']=array(array('gt',strtotime($gt_date)),array('lt',strtotime($lt_date))); } else if($lt_date) { $where['date']=array('lt',strtotime($lt_date)); } else if($gt_date) { $where['date']=array('gt',strtotime($gt_date)); } foreach($data as $k=>$v) { if($v)$where[$k]=$v; } $state=pg('state'); ?>
  <div class="right_panel" id="right">
    <form action="admin.php?m=order&a=index&admin_classify_id=21" method="post">
      ...
      <div class="remind clearfix">
        ...
        <ul>
          <?php
 $dfk_count=M('order')->where(array('state'=>1))->count(); $dfh_count=M('order')->where(array('state'=>2))->count(); $dsh_count=M('order')->where(array('state'=>3))->count(); ?>
    ...
    <form method="post" id="form_menu" action="admin.php?m=order&a=batch_edit_save" >
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ordertable">
        ...
        <?php
 $perpage=20; $offset=($p-1)*$perpage; if($search_type==''&& empty($gt_date) && empty($gt_date)) { $total_num= M('order')->where($where)->count(); $order=M('order')->where($where)->limit($offset,$perpage)->order('date desc')->select(); } else { if($gt_date) { $w_date=' and o.date >'.strtotime($gt_date); } if($lt_date) { $w_date.=' and o.date <'.strtotime($lt_date); } if( !$gt_date && !$lt_date){ $w_date=''; } $key=array_keys($data); if($key[0]=='goods_name'){ $total_num= M()->table('index_order o,index_order_goods og')->where('o.order_id = og.order_id and og.'.$key[0].' like "%'.$data[$key[0]].'%" '.$w_date)->count(); $order = M()->table('index_order o,index_order_goods og')->where('o.order_id = og.order_id and og.'.$key[0].' like "%'.$data[$key[0]].'%" '.$w_date)->select(); }else{ $total_num= M()->table('index_order o,index_order_goods og')->where('o.order_id = og.order_id and o.'.$key[0].' like "%'.$data[$key[0]].'%" '.$w_date)->count(); $order = M()->table('index_order o,index_order_goods og')->where('o.order_id = og.order_id and o.'.$key[0].' like "%'.$data[$key[0]].'%" '.$w_date)->select(); } } foreach($order as $k=>$v) { ?>
        <tbody>
          <tr height="30">
          ...
          <?php
 $order_goods=M('order_goods')->where(array('order_id'=>$v['order_id']))->select(); foreach($order_goods as $og_k=>$og_v) { ?>
          <tr>
            <td class="td1"><a href="index.php?m=goods&a=details&content_id=<?php echo $og_v['goods_id'];?>" class="fl"><img src="<?php echo $og_v['goods_img']?>" width="100" height="80"/></a>
              <div class="gsattr"> <a href="index.php?m=goods&a=details&goods_id=<?php echo $og_v['goods_id'];?>"><?php echo $og_v['goods_name'] ?></a>
                <div class="red">
				<?php
 $name=unserialize($og_v['name']); $value=unserialize($og_v['value']); foreach($name as $k2=>$v2) { echo $v2.':'.$value[$k2].'<br>'; } ?>
           </div>
              </div>
              
              </td>
            <td class="td2"><div class="consignee">
                <?php  $goods_attribute_array=unserialize($og_v['goods_attribute_array']); $goods_attribute_value_array=unserialize($og_v['goods_attribute_value_array']); foreach($goods_attribute_array as $k_gaa=>$v_gaa) { echo $v_gaa['name'].':'.$goods_attribute_value_array[$v_gaa[id]]['name'].'&nbsp;&nbsp;&nbsp;&nbsp;'; }?>
