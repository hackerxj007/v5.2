...
class siteAction extends Action {
	...
	public function password_select()
	{
		...
		if(!empty($user))
		{
			...
			$user=M('user')->where(array('password'=>$password,'user_id'=>$user['user_id']))->find();
			if(empty($user))
			{
	...
	public function password_edit_save()
	{
		...
		{
			$password=md5(pg('password'));
			$old_password=md5(pg('old_password'));
			$confirm_password=md5(pg('confirm_password'));
			$user=M('user')->where(array('password'=>$old_password,'user_id'=>$user['user_id']))->find();
			if($password==$confirm_password)
			{
				if(!empty($user))
				{
					M('user')->where(array('user_id'=>$user['user_id']))->save(array('password'=>$password));//修改密码、
